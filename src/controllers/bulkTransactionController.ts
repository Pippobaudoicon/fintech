...
export class BulkTransactionController {
  async createBulkTransaction(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const result = await bulkTransactionService.createBulkTransaction(req.user!.id, req.body);
      ...
      res.status(400).json({
        success: false,
        message: error instanceof Error ? error.message : 'Failed to create bulk transaction',
      });
    }
  }
  async getBulkTransactionStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({
      ...
      const status = await bulkTransactionService.getBulkTransactionStatus(batchId);
      ...
      res.status(404).json({
        success: false,
        message: error instanceof Error ? error.message : 'Bulk transaction batch not found',
      });
    }
  }
  async getUserBulkTransactions(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({
...
export const validateCreateBulkTransaction = [
  ...
  body('transactions.*.toAccountId').isString().notEmpty().withMessage('To account ID is required'),
