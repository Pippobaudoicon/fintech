...
class AuditService {
  async getAuditLogs(
    ...
    try {
      ...
      if (filters.severity) {
        ...
        logger.info(`Severity filter requested: ${filters.severity} (not yet implemented)`);
      }

      if (filters.category) {
        // Add category filter when we enhance the schema
        logger.info(`Category filter requested: ${filters.category} (not yet implemented)`);
      }
      ...
      return {
        logs: logs.map((log) => ({
          ...
          complianceRelevant: this.isComplianceRelevant(log.action, log.resource),
        })),
        pagination: {
          currentPage: page,
          totalPages,
  ...
  async exportAuditLogs(
    ...
    try {
      ...
      if (filters.action) whereClause.action = { contains: filters.action, mode: 'insensitive' };
      if (filters.resource)
        whereClause.resource = {
          contains: filters.resource,
          mode: 'insensitive',
      ...
      if (format === 'csv') {
        ...
        logger.info('XLSX format requested but not yet implemented, returning CSV');
        return this.generateCSV(logs);
      }
  ...
  async getAuditStatistics(days: number = 30) {
    try {
      ...
      const [totalLogs, userActions, actionBreakdown, resourceBreakdown, dailyActivity] =
        await Promise.all([
  ...
  private categorizeAuditLog(action: string, resource: string): string {
    ...
    if (resource.toLowerCase().includes('user') || resource.toLowerCase().includes('account')) {
